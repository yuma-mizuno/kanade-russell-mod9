...
identities needed to evaluate this polynomial at the source sums.
Section~\ref{sec:product} evaluates the cubic at the products using
the Jacobi addition formula and classical identities for cubic theta and Lambert series. Section~\ref{sec:positive-completion}
...
In the analytic arguments below, \(0<|q|<1\).
...
\begin{proposition}[Addition identities]\label{prop:app-addition}
...
\end{equation}
where \(R_{-1}=R_0+R_1\) follows from
\eqref{eq:app-R-contiguity}.

The positive source has the elementary contiguities
...
Reindexing the three coefficients on the right by
\(m-1,m-2,n-1\), respectively, reduces the required equality to
\[
 1-XY+\frac{1-X}{X}-\frac{(1+q)(1-X)Y}{q}
 -\frac{(1-X/q)(1-X)Y}{X}-\frac{1-Y}{X}=0.
\]
This rational identity is exact; the factors \(1-X\),
\(1-X/q\), and \(1-Y\) also supply the zero boundary terms.
...
The identities \eqref{eq:app-bilinear-certificate} define a bilinear
intertwiner from the two difference systems for \(\mathsf F\) to the
scalar difference equation for \(\mathsf G\).
The quadratic expressions in \eqref{eq:app-quadratic-bridge} arise
by specializing this intertwiner at \(x=1\).

\section{Casoratian}
\label{app:casoratian}

For \(0<|p|<1\), define the \(q\)-Airy function with base \(p\) by
\[
...
\]
satisfies \(\omega(pX)=\omega(X)\).  Iterating towards zero gives
\begin{equation}\label{eq:app-Ai-Wronskian}
 \omega(X)=\omega(0)=2.
\end{equation}

Choose \(t\) with \(t^4=q\), and define
\[
...
Thus \(\mathcal A,\mathcal B\) are independent of the choice of
fourth root after the parity separation.  Define the two rows
...
\(W_1(x)=x+O(x^2)\).  Put \(H(x)=W_1(x)/x\), with its
removable value \(H(0)=1\).
Taking two-row determinants in
\eqref{eq:app-scalar-equation} gives
\begin{align*}
...
by \(1-q^j\ne0\).  The analytic solution with \(H(0)=1\) is unique.
...
\end{equation}
It holds for \(X\ne0\).  The factor in the cited formula is
...
\section{Product cubic}\label{sec:product}
...
For an auxiliary variable $u$ and nome $\rho$, define
\[
...
\section{Cubic norm and source evaluations}
...
\[
 ...
 m^2+3mn+3n^2+2m+3n.
\]
These are exactly the defining exponents of the three sources.
...
so, in retrospect, I think this worked out well.
...
When GPT-6 Astra became available in September 2026, I asked, as usual, whether it could solve the conjecture.
...
that I had stated in my earlier paper, but I had not thought to consider it.
...
It then combined this equality with Tsuchioka's coefficientwise inequalities
